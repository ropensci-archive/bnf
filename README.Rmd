...
and generate code from that grammar.
...
* The current BNF parsing process is very rough It needs a refactor/rewrite to
  improve:
    * Robustness
    * Correctness
    * Completeness
    * Readability
* Need to document the particular idiosyncratic flavour of BNF that is supported e.g. must have a trailing
  `;` after each rule
* Work on a more complete R grammar in order to generate more interesting programs.
* More tunable code generation parameters
    * e.g. generation of "zero or more" elements samples from a poisson distribution,
      but the lambda for this distribution is set globally. It would be nice to
      be separately configurable for each rule.


## Installation

Install the development version from [GitHub](https://github.com/). This will also
require installing [minilexer](https://github.com/coolbutuseless/minilexer) which
is used in parsing the original BNF grammar
...
<details>
<summary>Click here to show/hide the BNF spec as an R object (a deeply nested list)</summary>
